...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: BaseInputReader, text_encoder: BertTokenizer,
        ...
        # relations
        self._gt_relations = []  # ground truth 真实值
        self._pred_relations = []  # prediction 预测值

        # entities
        self._gt_entities = []  # ground truth 真实值
        self._pred_entities = []  # prediction 预测值

        self._pseudo_entity_type = EntityType('Entity', 1, 'Entity', 'Entity')  # 虚假对象 for span only evaluation

        self._convert_gt(self._dataset.documents)  # 构造真实的的关系和实体对象
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor,
        ...
        batch_pred_entities, batch_pred_relations = prediction.convert_predictions(
            batch_entity_clf,  # 前向过程预测的各个实体对应的类别概率softmax
            batch_rel_clf,  # 前向过程预测的各个关系对应的类别概率sigmoid
            batch_rels,  # 各个关系对应的实体下标
            batch,  # 批次数据对象
            self._rel_filter_threshold,  # 关系预测的时候的概率阈值
            self._input_reader,  # 数据加载器
            no_overlapping=self._no_overlapping  # 是否允许有重叠
        )
    ...
    def compute_scores(self):
        ...
        print("")
        _gt_entities = self._gt_entities[:len(self._pred_entities)]  # 正常不应该进行截断操作，这里只是因为前面做了break操作
        gt, pred = self._convert_by_setting(_gt_entities, self._pred_entities, include_entity_types=True)
        ...
        print("")
        _gt_relations = self._gt_relations[:len(self._pred_relations)]
        gt, pred = self._convert_by_setting(_gt_relations, self._pred_relations, include_entity_types=False)
        ...
        print("")
        _gt_relations = self._gt_relations[:len(self._pred_relations)]
        gt, pred = self._convert_by_setting(_gt_relations, self._pred_relations, include_entity_types=True)
    ...
    def _convert_gt(self, docs: List[Document]):
        for doc in docs:
            gt_relations = doc.relations  # 获取得到当前文本包含的关系列表
            gt_entities = doc.entities  # 获取得到当前文本包含的实体列表
    ...
    def _convert_by_setting(self, gt: List[List[Tuple]], pred: List[List[Tuple]],
        ...
        def convert(t):
            if not include_entity_types:
                ...
                if type(t[0]) == int:  # entity # 不考虑实体类型，只考虑位置
                    c = [t[0], t[1], self._pseudo_entity_type]
                else:  # relation 不考虑实体类型，仅考虑位置以及关系类别
        ...
        for sample_gt, sample_pred in zip(gt, pred):
            converted_gt.append([convert(t) for t in sample_gt])  # 针对当前样本/doc对应的所有真实值进行转换
            converted_pred.append([convert(t) for t in sample_pred])  # 针对当前样本/doc对应的所有预测值进行转换
    ...
    def _score(self, gt: List[List[Tuple]], pred: List[List[Tuple]], print_results: bool = False):
        ...
        for (sample_gt, sample_pred) in zip(gt, pred):  # 遍历每条样本，处理对应的真实值和预测值
            # 真实值和预测值的并集
            union = set()
            ...
            for s in union:
                if s in sample_gt:
                    ...
                    gt_flat.append(0)  # 如果当前对象不在sample_gt中，那么表示当前对象是预测错了，类别直接用0表示
                ...
                if s in sample_pred:
                    ...
                    pred_flat.append(0)  # 如果当前对象不在sample_pred中，那么表示当前对象没有被预测出来，预测错误，直接用0表示
    ...
    def _compute_metrics(self, gt_all, pred_all, types, print_results: bool = False):
        ...
        total_support = sum(per_type[-1])  # 所有类别，真实标签数目
