...
def padded_stack(tensors, padding=0):
    dim_count = len(tensors[0].shape)  # 获取有多少维

    max_shape = [max([t.shape[d] for t in tensors]) for d in range(dim_count)]  # 获取每个维度上的最大长度
    padded_tensors = []
...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        _model_path = model_path if model_path.endswith('.bin') else os.path.join(model_path, 'pytorch_model.bin')
        if not os.path.exists(_model_path):
            _model_path = os.path.join(model_path, 'model.safetensors')
        if _model_path.endswith(".bin"):
            state_dict = torch.load(_model_path, map_location=torch.device('cpu'))
        else:
            from safetensors.torch import load_file
            state_dict = load_file(_model_path, device='cpu')
...
def build_text_to_tokens_func(spacy_model: str = None):
    """
    https://spacy.io/
    ...
    _nlp_model = spacy.load(spacy_model) if spacy is not None and spacy_model is not None else None

    def _nlp(document):
        if _nlp_model is not None:
            return [t.text for t in _nlp_model(document)]
        else:
            return document.split(" ")  # 针对空格隔开的字符串处理
            # return list(document)  # 针对纯中文的处理
