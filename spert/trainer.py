...
class BaseTrainer:
    ...
    def __init__(self, args: argparse.Namespace):
        self._args = args  # 参数对象
        self._debug = self._args.debug

        run_key = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")  # 随机的一个时间字符串
    ...
    def _save_model(self, save_path: str, model: PreTrainedModel, tokenizer: PreTrainedTokenizer,
                    ...
                    extra: dict = None, include_iteration: int = True, name: str = 'model', safe_serialization=True):
        ...
        if isinstance(model, DataParallel):
            ...
            model.save_pretrained(dir_path, safe_serialization=safe_serialization)
