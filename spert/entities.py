...
class RelationType:
    def __init__(self, identifier, index, short_name, verbose_name, symmetric=False):
        self._identifier = identifier  # 唯一标识字符串
        self._index = index  # 关系对应的索引id
        self._short_name = short_name  # 关系的端名称
        self._verbose_name = verbose_name  # 描述字符串
        self._symmetric = symmetric  # 这个关系是不是双向的
...
class EntityType:
    def __init__(self, identifier, index, short_name, verbose_name):
        self._identifier = identifier  # 唯一名词字符串
        self._index = index  # id
        self._short_name = short_name  # 短名称
        self._verbose_name = verbose_name  # 描述语句
...
class Token:
    def __init__(self, tid: int, index: int, span_start: int, span_end: int, phrase: str):
        ...
        self._span_start = span_start  # start of token span in document (inclusive) 开始下标
        self._span_end = span_end  # end of token span in document (exclusive) 结尾下标
        self._phrase = phrase
...
class Document:
    def __init__(self, doc_id: int, tokens: List[Token], entities: List[Entity], relations: List[Relation],
        ...
        self._tokens = tokens  # 当前文本对应的所有token列表
        self._entities = entities  # 当前文本对应的实体列表
        self._relations = relations  # 当前文本对应的关系列表

        # byte-pair document encoding including special tokens ([CLS] and [SEP])
        self._encoding = encoding  # 进行byte-pair/word piece转换处理后的token id列表
...
class Dataset(TorchDataset):
    ...
    def __init__(self, label, rel_types, entity_types, neg_entity_count,
        ...
        self._label = label
        self._rel_types = rel_types  # 关系元数据
        self._entity_types = entity_types  # 实体元数据
        self._neg_entity_count = neg_entity_count
        self._neg_rel_count = neg_rel_count
        self._max_span_size = max_span_size
        self._mode = Dataset.TRAIN_MODE

        self._documents = OrderedDict()  # 当前数据集的文档字典
        self._entities = OrderedDict()  # 当前数据集的实体字典
        self._relations = OrderedDict()  # 当前数据集的关系字典

        # current ids
        self._doc_id = 0  # 文档/文本id
        self._rid = 0  # relation关系id
        self._eid = 0  # entity实体id
        self._tid = 0  # token id
    ...
    def create_entity(self, entity_type, tokens, phrase) -> Entity:
        ...
        self._entities[self._eid] = mention  # 当前Dataset对象中保持实体对象
        self._eid += 1
        return mention
    ...
    def __getitem__(self, index: int):
        doc = self._documents[index]  # 获取当前文本对象，包含token span、token id、entity、relation

        if self._mode == Dataset.TRAIN_MODE:
            return sampling.create_train_sample(
                doc,  # 当前文本对象
                self._neg_entity_count,  # 实体的负样本数量
                self._neg_rel_count,  # 关系的负样本数量
                self._max_span_size,  # 实体允许的最大token序列长度
                len(self._rel_types)  # 元数据-关系的数量
            )
        else:
